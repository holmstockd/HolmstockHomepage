...
/* ── MySQL-configured check ────────────────────────────────────────── */
function _loginMysqlConfigured(): bool {
    $c = @file_get_contents(__DIR__ . '/dash_config.php') ?: '';
    return strpos($c, "DASH_DB_TYPE', 'mysql'") !== false;
}

/* ── Sub-user loader — MySQL-strict when configured ───────────────── */
function getSubUsersLogin(): array {
    if (!function_exists('getDashDb')) @include_once __DIR__ . '/db.php';
    if (function_exists('getDashDb') && function_exists('dashGetUsers')) {
        $db = getDashDb();
        if ($db) return dashGetUsers($db);
    }
    // If MySQL is configured but connection failed → return empty (no JSON fallback for security)
    if (_loginMysqlConfigured()) return [];
    $f = __DIR__ . '/dash_users.json';
    return json_decode(@file_get_contents($f) ?: '[]', true) ?: [];
}

function verifySubUserLogin(string $user, string $pass): ?array {
    if (!function_exists('getDashDb')) @include_once __DIR__ . '/db.php';
    if (function_exists('getDashDb') && function_exists('dashVerifyUser')) {
        $db = getDashDb();
        if ($db) return dashVerifyUser($db, $user, $pass);
    }
    if (_loginMysqlConfigured()) return null;
    foreach (getSubUsersLogin() as $su) {
        if ($su['username'] === $user && password_verify($pass, $su['password_hash'])) return $su;
    }
    return null;
}
