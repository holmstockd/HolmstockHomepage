...
if (!file_exists(__DIR__ . '/dash_config.php')) {
    http_response_code(503); header('Content-Type: application/json'); echo '{"error":"Not configured"}'; exit;
}
if (empty($_SESSION['logged_in'])) {
    if (isset($_COOKIE['dash_auth'])) @include_once __DIR__ . '/auth.php';
    if (empty($_SESSION['logged_in'])) {
        http_response_code(401); header('Content-Type: application/json'); echo '{"error":"Not configured or not authenticated"}'; exit;
    }
}
